...
# Create a dataset class for the best_feature.txt file
...
class Feature_dataset(Dataset):
    ...
    def parse_annotation(self):
        # Load the best_features.txt file
    ...
    def __len__(self):
        return len(self.parse_annotation())

    def __getitem__(self, item):
        # Get the image path and the features
        annotation = self.parse_annotation()[item]
        ...
        features = np.array(line[1].split(',')).astype(np.float32)
...
class VocDataset(Dataset):
    ...
    def __load_annotations(self, anno_type):
        ...
        assert anno_type in ['foggy','train','test'], "You must choice one of the 'train' or 'test' for anno_type parameter"
        anno_path = os.path.join(cfg.PROJECT_PATH, 'data', 'data_fog', anno_type+"_annotation.txt")
