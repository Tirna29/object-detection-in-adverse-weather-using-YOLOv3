...
def name_to_bgr(color_name):
    rgb = np.array(COLOR_NAME_TO_RGB[color_name])
    rgb = np.uint8(rgb.reshape(1,1,3))  # Convert to 8-bit RGB for cv2
    bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
    return tuple(map(int, bgr[0,0,:]))

# Convert color names to BGR values
STANDARD_COLORS = [name_to_bgr(color) for color in COLOR_NAME_TO_RGB]
...
def draw_bounding_box_on_image_array(image,
    ...
    """Draws a bounding box on an image (numpy array)."""
    ...
    for i, display_str in enumerate(display_str_list):
      cv2.putText(image, display_str, (int(xmin), int(ymin) - 10 * i), font, font_scale, color, 1, cv2.LINE_AA)
